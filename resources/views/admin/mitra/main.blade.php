...
<head>
    ...
    <title> KEMS | Key Encryption & Security Management</title>
...
<body>
    <div class="wrapper">
        ...
        @include("admin.mitra.topbar")

        @include("admin.mitra.sidebar")
    ...
    <script>
        $(document).ready(function() {
            $('#id_sentral').change(function() {
                $.ajax({
                    type:"GET",
                    dataType: "json",
                    data:{idSentral: $(this).val()},
                    url:"{{ url('/api/getKunciSentral') }}",
                    success:function(data){
                        $('#kuncis_id').empty();
                        if (data.length < 1) {
                            $('#kuncis_id').append("<option disabled selected>Tidak Ada Kunci</option>");
                        } else {
                            data.forEach(function(item){
                                $('#kuncis_id').append("<option value="+item.id+">"+item.kuncis_id+"</option>");
                            });
                        }
                    }
                });
